...
ImageLoad(char *filename, Image *image) {
    ...
    image->sizeX = (unsigned long) sizex;
    image->sizeY = (unsigned long) sizey;
    size = image->sizeX * image->sizeY * 3;
	printf("LOADER Voici les tailles\t %lu %lu => %lu\n", image->sizeX, image->sizeY, size);
	if (! infoheader.width)
	  exit(EXIT_FAILURE);
...
ImageSave(char *filename, Image *image) {
    ...
    struct {
      unsigned short int type;                 /* Magic identifier            */
      GLubyte size;                       /* File size in bytes          */  
      unsigned short int reserved1, reserved2;
      unsigned int offset;                     /* Offset to image data, bytes */
    } header;
